import math
import torch
import torch.nn as nn
from torch.autograd import Variable
import numpy as np
import torch.nn.functional as F
import scipy.stats as st
import torchvision
from functools import partial

import time
from losses import LogitLoss
# from odi import render_3d_aug_input, Render3D
render_3d_aug_input, Render3D = None, None

class Base(object):
    def __init__(self, loss_fn, eval_steps, steps, target=False, random_start=False, epsilon=16./255., alpha=2./255.):
        self.eval_steps = eval_steps   
        self.steps = steps
        self.target = target
        self.random_start = random_start
        self.epsilon = epsilon
        self.alpha = alpha
        self.loss_fn_name = loss_fn
        if loss_fn == 'CE':
            self.loss_fn = nn.CrossEntropyLoss()
        elif loss_fn == 'Logit':
            self.loss_fn = LogitLoss()
        elif loss_fn == 'Attention':
            self.loss_fn = None
        self.begin_time = time.time()

    def _DI(self, X_in, di_prob=0.7):
        rnd = np.random.randint(299, 330,size=1)[0]
        h_rem = 330 - rnd
        w_rem = 330 - rnd
        pad_top = np.random.randint(0, h_rem,size=1)[0]
        pad_bottom = h_rem - pad_top
        pad_left = np.random.randint(0, w_rem,size=1)[0]
        pad_right = w_rem - pad_left

        c = np.random.rand(1)
        if c <= di_prob:
            X_out = F.pad(F.interpolate(X_in, size=(rnd,rnd)),(pad_left,pad_right,pad_top,pad_bottom),mode='constant', value=0)
            return  X_out 
        else:
            return  X_in

    def _DI_WO_Prob(self, X_in):
        rnd = np.random.randint(299, 330,size=1)[0]
        h_rem = 330 - rnd
        w_rem = 330 - rnd
        pad_top = np.random.randint(0, h_rem,size=1)[0]
        pad_bottom = h_rem - pad_top
        pad_left = np.random.randint(0, w_rem,size=1)[0]
        pad_right = w_rem - pad_left

        X_out = F.pad(F.interpolate(X_in, size=(rnd,rnd)),(pad_left,pad_right,pad_top,pad_bottom),mode='constant', value=0)
        # X_out = F.interpolate(X_out, size=(X_in.shape[-2], X_in.shape[-1]))
        return  X_out 

    def _TI_kernel(self):
        def gkern(kernlen=15, nsig=3):
            x = np.linspace(-nsig, nsig, kernlen)
            kern1d = st.norm.pdf(x)
            kernel_raw = np.outer(kern1d, kern1d)
            kernel = kernel_raw / kernel_raw.sum()
            return kernel
        channels=3
        kernel_size=5
        kernel = gkern(kernel_size, 3).astype(np.float32)
        gaussian_kernel = np.stack([kernel, kernel, kernel])
        gaussian_kernel = np.expand_dims(gaussian_kernel, 1)
        gaussian_kernel = torch.from_numpy(gaussian_kernel).cuda()
        return gaussian_kernel
    
    def _target_layer(self, model_name, depth):
        '''
        'inception_v3', 'resnet50', 'densenet121', 'vgg16_bn'
        depth: [1, 2, 3, 4]
        '''
        if model_name == 'resnet50':
            return getattr(self.model[1], 'layer{}'.format(depth))[-1]
        elif model_name == 'vgg16_bn':
            depth_to_layer = {1:12,2:22,3:32,4:42}
            return getattr(self.model[1], 'features')[depth_to_layer[depth]]
        elif model_name == 'densenet121':
            return getattr(getattr(self.model[1], 'features'), 'denseblock{}'.format(depth))
        elif model_name == 'inception_v3':
            depth_to_layer = {1:'Conv2d_4a_3x3', 2:'Mixed_5d', 3:'Mixed_6e', 4:'Mixed_7c'}
            return getattr(self.model[1], '{}'.format(depth_to_layer[depth]))

    def perturb(self, images, ori_labels, target_labels):
        raise NotImplementedError

class DTMI(Base):
    '''
    baseline_cmd: This is a string containing one or more baseline attack options separated by underscores, e.g., "DI_TI_MI" or "ODI_RAP". Accepts one of six values: 'DI', 'TI', 'MI', 'ODI', 'RAP' or 'VT'.
    linear_aug: This controls the choice of linear augmentation strategy used in the attack. Accepts one of three values: 'SI', 'Admix', or 'Ours'. Note that 'Ours' means Dynamic LA.
    '''
    def __init__(self, baseline_cmd, linear_aug, model, eval_models, loss_fn, eval_steps, steps, target):
        super(DTMI, self).__init__(loss_fn, eval_steps, steps, target)
        self.model = model
        self.eval_models = eval_models
        
        for baseline_opt in ['DI', 'TI', 'MI', 'ODI', 'RAP', 'VT']:
            if baseline_opt in baseline_cmd.split('_'):
                setattr(self, baseline_opt, True)
            else:
                setattr(self, baseline_opt, False)

        for linear_opt in ['SI', 'Admix', 'Ours']:
            if linear_opt == linear_aug:
                setattr(self, linear_opt, True)
            else:
                setattr(self, linear_opt, False)

        if self.Ours:
            self.gamma = torch.ones(1)
            self.eta = torch.zeros(1)
        
        if self.ODI:
            self.render = Render3D(101, 0)
            self.prob = 1.0

        if self.VT:
            # following https://github.com/JHL-HUST/VT/blob/main/vmi_fgsm.py
            self.vt_number = 20
            self.vt_beta = 1.5

        if self.TI:
            self.gaussian_kernel = self._TI_kernel()

    def _vt_batch_grad(self, model, x_adv, y):
        vt_gradients = torch.zeros_like(x_adv)
        for vt_itr in range(self.vt_number):
            x_neighbor = x_adv + torch.FloatTensor(*x_adv.shape).uniform_(-self.vt_beta*self.epsilon, self.vt_beta*self.epsilon).cuda()
            x_neighbor.requires_grad_()
            logits = model(x_neighbor)
            loss = self.loss_fn(logits, y)
            grad_c = torch.autograd.grad(loss, x_neighbor,
                                        retain_graph=False, create_graph=False)[0]
            vt_gradients += grad_c
        return vt_gradients/self.vt_number

    def _pgd_adv_example(self, model,
             x_natural,
             y,
             step_size=2/255,
             epsilon=16/255,
             perturb_steps=8,
             random_start=True):
        ''' refer to https://github.com/dongyp13/memorization-AT/blob/main/train.py'''
        batch_size = len(x_natural)
        # generate adversarial example
        if random_start:
            x_adv = x_natural.clone().detach() + torch.FloatTensor(*x_natural.shape).uniform_(-epsilon, epsilon).cuda()
        else:
            x_adv = x_natural.clone().detach()
        x_adv = torch.clamp(x_adv, 0.0, 1.0)
        for _ in range(perturb_steps):
            x_adv.requires_grad_()
            with torch.enable_grad():
                loss = F.cross_entropy(model(x_adv), y)
            grad = torch.autograd.grad(loss, [x_adv])[0]
            x_adv = x_adv.detach() + step_size * torch.sign(grad.detach())
            x_adv = torch.min(torch.max(x_adv, x_natural - epsilon), x_natural + epsilon)
            x_adv = torch.clamp(x_adv, 0.0, 1.0)
        pert = x_adv - x_natural
        return pert

    def _cal_coef(self, para, mode, lower, upper, itr):
        if mode == 'uniform':
            return para.uniform_(lower, upper).item()
        elif mode == 'increment_linear':
            return torch.linspace(lower+1e-3, upper, self.steps, dtype = torch.float64)[itr].item()

    def _ODI(self, inputs):
        inputs = render_3d_aug_input(inputs, renderer=self.render, prob=self.prob)
        return inputs

    def _linear_aug(self, inputs, itr):
        if self.SI:
            inputs = torch.cat([inputs/(2**i) for i in range(5)], dim=0)
        if self.Admix:
            inputs = torch.cat([inputs + 0.2 * inputs[torch.randperm(inputs.size(0))] for i in range(5)], dim=0)
        if self.Ours:
            our_inputs = []
            for our_i in range(5):
                gamma = self._cal_coef(self.gamma, 'uniform', 0.0, 1.8, itr)
                eta = self._cal_coef(self.eta, 'increment_linear', 0.0, 1.8, itr)
                noise = torch.empty_like(inputs).uniform_(-self.epsilon, self.epsilon)
                our_inputs.append(torch.clamp(gamma*(inputs) + eta*noise, min=0, max=1))
            inputs = torch.cat(our_inputs, dim=0)
        return inputs

    def perturb(self, images, ori_labels, target_labels):
        b,c,h,w = images.shape

        adv_images = images.clone()
        if self.target:
            used_label = target_labels
            used_coef = -1
        else:
            used_label = ori_labels
            used_coef = 1
        
        if self.random_start:
            adv_images = adv_images + torch.empty_like(adv_images).uniform_(-self.epsilon, self.epsilon)
            adv_images = torch.clamp(adv_images, min=0, max=1).detach()
            delta = torch.tensor((adv_images - images).clone().detach(), requires_grad=True).cuda()
        else:
            delta = torch.zeros_like(adv_images,requires_grad=True).cuda()

        pert = torch.zeros_like(adv_images)
        result_matrix = np.zeros((len(self.eval_models), len(self.eval_steps)))
        iter_flag = 0
        if self.MI:
            grad_pre = 0
        if self.VT:
            vt_variance = torch.zeros_like(adv_images)

        for itr in range(self.steps):
            delta.requires_grad_()

            if self.RAP:
                pert = self._pgd_adv_example(self.model, adv_images+delta.detach().clone(), used_label)
            reverse_adv_inputs = adv_images + pert + delta
            
            if self.ODI:
                reverse_adv_inputs = self._ODI(reverse_adv_inputs)

            augmented_inputs = self._linear_aug(reverse_adv_inputs, itr)

            loss_label = torch.cat([used_label]*int(augmented_inputs.shape[0]/b))
            
            if self.DI:
                logits = self.model(self._DI(augmented_inputs))
            else:
                logits = self.model(augmented_inputs)
            loss = self.loss_fn(logits, loss_label)
            grad_c = torch.autograd.grad(loss, delta,
                                        retain_graph=False, create_graph=False)[0]
            if self.TI:
                grad_c = F.conv2d(grad_c, self.gaussian_kernel, bias=None, stride=1, padding=(2,2), groups=3)
            else:
                pass
            
            if self.VT:
                grad_c = grad_c + vt_variance

            if self.MI:
                grad_a = grad_c / torch.mean(torch.abs(grad_c), (1, 2, 3), keepdim=True) + 1 * grad_pre
                grad_pre = grad_a
            else:
                grad_a = grad_c.clone()

            delta = delta + used_coef * self.alpha * torch.sign(grad_a)
            delta = delta.clamp(-self.epsilon, self.epsilon) 
            delta = ((adv_images + delta).clamp(0,1)) - adv_images

            if self.VT:
                vt_gradients = self._vt_batch_grad(self.model, adv_images+delta.detach().clone(), used_label)
                vt_variance = vt_gradients - grad_a

            if itr+1 in self.eval_steps:
                for m_id, model in enumerate(self.eval_models):
                    with torch.no_grad():
                        this_logit = model(adv_images+delta)
                    if self.target:
                        success_nums = torch.sum(torch.argmax(this_logit, dim=1) == used_label).cpu().item()
                    else:
                        success_nums = torch.sum(torch.argmax(this_logit, dim=1) != used_label).cpu().item()
                    result_matrix[m_id, iter_flag] = success_nums
                iter_flag += 1
            torch.cuda.empty_cache()
            
        return result_matrix, adv_images+delta

class DTMI_Untargeted(Base):
    '''
    It is specifically designed for non-targeted attacks.
    '''
    def __init__(self, baseline_cmd, linear_aug, model, eval_models, loss_fn, eval_steps, steps, target):
        super(DTMI_Untargeted, self).__init__(loss_fn, eval_steps, steps, target)
        self.model = model
        self.eval_models = eval_models
        
        for baseline_opt in ['DI', 'TI', 'MI', 'VT']:
            if baseline_opt in baseline_cmd.split('_'):
                setattr(self, baseline_opt, True)
            else:
                setattr(self, baseline_opt, False)

        for linear_opt in ['SI', 'Admix', 'Ours']:
            if linear_opt == linear_aug:
                setattr(self, linear_opt, True)
            else:
                setattr(self, linear_opt, False)

        if self.Ours:
            self.gamma = torch.ones(1)
            self.eta = torch.zeros(1)
        

        if self.VT:
            # following https://github.com/JHL-HUST/VT/blob/main/vmi_fgsm.py
            self.vt_number = 20
            self.vt_beta = 1.5

        if self.TI:
            self.gaussian_kernel = self._TI_kernel()

    def _vt_batch_grad(self, model, x_adv, y):
        vt_gradients = torch.zeros_like(x_adv)
        for vt_itr in range(self.vt_number):
            x_neighbor = x_adv + torch.FloatTensor(*x_adv.shape).uniform_(-self.vt_beta*self.epsilon, self.vt_beta*self.epsilon).cuda()
            x_neighbor.requires_grad_()
            logits = model(x_neighbor)
            loss = self.loss_fn(logits, y)
            grad_c = torch.autograd.grad(loss, x_neighbor,
                                        retain_graph=False, create_graph=False)[0]
            vt_gradients += grad_c
        return vt_gradients/self.vt_number

    def _cal_coef(self, para, mode, lower, upper, itr):
        if mode == 'uniform':
            return para.uniform_(lower, upper).item()
        elif mode == 'decrement_linear':
            return torch.linspace(upper, lower, self.steps, dtype = torch.float64)[itr].item()

    def _linear_aug(self, inputs, itr):
        if self.SI:
            inputs = torch.cat([inputs/(2**i) for i in range(5)], dim=0)
        if self.Admix:
            inputs = torch.cat([inputs + 0.2 * inputs[torch.randperm(inputs.size(0))] for i in range(5)], dim=0)
        if self.Ours:
            our_inputs = []
            for our_i in range(5):
                gamma = self._cal_coef(self.gamma, 'decrement_linear', 0.0, 1.0, itr)
                eta = self._cal_coef(self.eta, 'decrement_linear', 0.0, 1.8, itr)
                noise = torch.empty_like(inputs).uniform_(-self.epsilon, self.epsilon)
                our_inputs.append(torch.clamp(gamma*(inputs) + eta*noise, min=0, max=1))
            inputs = torch.cat(our_inputs, dim=0)
        return inputs

    def perturb(self, images, ori_labels, target_labels):
        b,c,h,w = images.shape

        adv_images = images.clone()
        if self.target:
            used_label = target_labels
            used_coef = -1
        else:
            used_label = ori_labels
            used_coef = 1
        
        if self.random_start:
            adv_images = adv_images + torch.empty_like(adv_images).uniform_(-self.epsilon, self.epsilon)
            adv_images = torch.clamp(adv_images, min=0, max=1).detach()
            delta = torch.tensor((adv_images - images).clone().detach(), requires_grad=True).cuda()
        else:
            delta = torch.zeros_like(adv_images,requires_grad=True).cuda()

        result_matrix = np.zeros((len(self.eval_models), len(self.eval_steps)))
        iter_flag = 0
        if self.MI:
            grad_pre = 0
        if self.VT:
            vt_variance = torch.zeros_like(adv_images)

        for itr in range(self.steps):
            delta.requires_grad_()

            reverse_adv_inputs = adv_images + delta

            augmented_inputs = self._linear_aug(reverse_adv_inputs, itr)
            loss_label = torch.cat([used_label]*int(augmented_inputs.shape[0]/b))
            
            if self.DI:
                logits = self.model(self._DI(augmented_inputs))
            else:
                logits = self.model(augmented_inputs)
            loss = self.loss_fn(logits, loss_label)
            grad_c = torch.autograd.grad(loss, delta,
                                        retain_graph=False, create_graph=False)[0]

            if self.VT:
                grad_c = grad_c + vt_variance

            if self.TI:
                grad_c = F.conv2d(grad_c, self.gaussian_kernel, bias=None, stride=1, padding=(2,2), groups=3)
            else:
                pass   

            if self.MI:
                grad_a = grad_c / torch.mean(torch.abs(grad_c), (1, 2, 3), keepdim=True) + 1 * grad_pre
                grad_pre = grad_a
            else:
                grad_a = grad_c.clone()

            delta = delta + used_coef * self.alpha * torch.sign(grad_a)
            delta = delta.clamp(-self.epsilon, self.epsilon) 
            delta = ((adv_images + delta).clamp(0,1)) - adv_images

            if self.VT:
                vt_gradients = self._vt_batch_grad(self.model, adv_images+delta.detach().clone(), used_label)
                vt_variance = vt_gradients - grad_a

            if itr+1 in self.eval_steps:
                for m_id, model in enumerate(self.eval_models):
                    with torch.no_grad():
                        this_logit = model(adv_images+delta)
                    if self.target:
                        success_nums = torch.sum(torch.argmax(this_logit, dim=1) == used_label).cpu().item()
                    else:
                        success_nums = torch.sum(torch.argmax(this_logit, dim=1) != used_label).cpu().item()
                    result_matrix[m_id, iter_flag] = success_nums
                iter_flag += 1
        return result_matrix, adv_images+delta


class DTMI_DilatedAttention(Base):
    '''
    The proposed method.
    '''
    def __init__(self, layer_names, target_layer, baseline_cmd, linear_aug, model, eval_models, loss_fn, eval_steps, steps, target):
        super(DTMI_DilatedAttention, self).__init__(loss_fn, eval_steps, steps, target)
        self.model = model
        self.eval_models = eval_models
        self.layer_names = layer_names
        self.target_layer = target_layer

        for baseline_opt in ['DI', 'TI', 'MI', 'ODI', 'RAP', 'VT']:
            if baseline_opt in baseline_cmd.split('_'):
                setattr(self, baseline_opt, True)
            else:
                setattr(self, baseline_opt, False)

        for linear_opt in ['SI', 'Admix', 'Ours']:
            if linear_opt == linear_aug:
                setattr(self, linear_opt, True)
            else:
                setattr(self, linear_opt, False)

        if self.Ours:
            self.gamma = torch.ones(1)
            self.eta = torch.zeros(1)
        
        if self.ODI:
            self.render = Render3D(101, 0)
            self.prob = 1.0

        if self.VT:
            # following https://github.com/JHL-HUST/VT/blob/main/vmi_fgsm.py
            self.vt_number = 20
            self.vt_beta = 1.5

        if self.TI:
            self.gaussian_kernel = self._TI_kernel()

        if loss_fn == 'Attention':
            self._attention_hook()


    def _initilize_gra_act(self):
        self.gradients = dict()
        self.activations = dict()

    def _attention_hook(self):
        self._initilize_gra_act()
        def backward_hook(module, grad_input, grad_output, layer_name):
            self.gradients[layer_name] = grad_output[0]
            return None
        def forward_hook(module, input, output, layer_name):
            self.activations[layer_name] = output
            return None
        if isinstance(self.target_layer, list):
            for i,j in zip(self.target_layer, self.layer_names):
                fuc_forward = partial(forward_hook, layer_name = j)
                fuc_backward = partial(backward_hook, layer_name = j)
                i.register_forward_hook(fuc_forward)
                i.register_backward_hook(fuc_backward) # register_full_backward_hook
        else:
            fuc_forward = partial(forward_hook, layer_name = self.layer_names)
            fuc_backward = partial(backward_hook, layer_name = self.layer_names)   
            self.target_layer.register_forward_hook(fuc_forward)
            self.target_layer.register_backward_hook(fuc_backward)

    def _attention_map(self, logits, delta, labels, retain_graph=True, create_graph=False):
        # return sum_losses (num_layers, num_images)
        # refer to https://stackoverflow.com/questions/54727099/activation-gradient-penalty
        attention_b, c, h, w = delta.size()
        score = logits.gather(1,labels.unsqueeze(1)).squeeze(1).sum()
        self.model.zero_grad()
        sum_losses = []
        for layer_name in self.layer_names:
            grad = torch.autograd.grad(score, self.activations[layer_name], create_graph=True, only_inputs=True)[0]
            act = self.activations[layer_name]
            grad_b, k, u, v = grad.size()
            alpha = grad.view(grad_b, k, -1).mean(2)
            weights = alpha.view(grad_b, k, 1, 1)

            saliency_map = (weights*act).sum(1, keepdim=True)
            sum_losses.append(saliency_map.view(grad_b,-1).sum(-1))
            # sum_losses.append(saliency_map.sum())
            # saliency_map = saliency_map / 0.5
            # out_sm = torch.sign(saliency_map) * torch.sqrt(torch.abs(saliency_map))
            # saliency_map = F.relu(saliency_map)
            # ignore upsample
            # saliency_map = F.upsample(saliency_map, size=(h, w), mode='bilinear', align_corners=False)
            # saliency_map_min, saliency_map_max = saliency_map.min(), saliency_map.max()
            # saliency_map = (saliency_map - saliency_map_min).div(saliency_map_max - saliency_map_min + 1e-6)
        sum_losses = torch.stack(sum_losses)
        return sum_losses

    def _vt_batch_grad(self, model, x_adv, y):
        vt_gradients = torch.zeros_like(x_adv)
        for vt_itr in range(self.vt_number):
            x_neighbor = x_adv + torch.FloatTensor(*x_adv.shape).uniform_(-self.vt_beta*self.epsilon, self.vt_beta*self.epsilon).cuda()
            x_neighbor.requires_grad_()
            logits = model(x_neighbor)
            loss = self.loss_fn(logits, y)
            grad_c = torch.autograd.grad(loss, x_neighbor,
                                        retain_graph=False, create_graph=False)[0]
            vt_gradients += grad_c
        return vt_gradients/self.vt_number

    def _pgd_adv_example(self, model,
             x_natural,
             y,
             step_size=2/255,
             epsilon=16/255,
             perturb_steps=8,
             random_start=True):
        ''' refer to https://github.com/dongyp13/memorization-AT/blob/main/train.py'''
        batch_size = len(x_natural)
        # generate adversarial example
        if random_start:
            x_adv = x_natural.clone().detach() + torch.FloatTensor(*x_natural.shape).uniform_(-epsilon, epsilon).cuda()
        else:
            x_adv = x_natural.clone().detach()
        x_adv = torch.clamp(x_adv, 0.0, 1.0)
        for _ in range(perturb_steps):
            x_adv.requires_grad_()
            with torch.enable_grad():
                loss = F.cross_entropy(model(x_adv), y)
            grad = torch.autograd.grad(loss, [x_adv])[0]
            x_adv = x_adv.detach() + step_size * torch.sign(grad.detach())
            x_adv = torch.min(torch.max(x_adv, x_natural - epsilon), x_natural + epsilon)
            x_adv = torch.clamp(x_adv, 0.0, 1.0)
        pert = x_adv - x_natural
        return pert

    def _cal_coef(self, para, mode, lower, upper, itr):
        if mode == 'uniform':
            return para.uniform_(lower, upper).item()
        elif mode == 'increment_linear':
            return torch.linspace(lower+1e-3, upper, self.steps, dtype = torch.float64)[itr].item()

    def _ODI(self, inputs):
        inputs = render_3d_aug_input(inputs, renderer=self.render, prob=self.prob)
        return inputs

    def _linear_aug(self, inputs, itr):
        if self.SI:
            inputs = torch.cat([inputs/(2**i) for i in range(5)], dim=0)
        if self.Admix:
            # inputs = torch.cat([inputs + 0.2 * inputs[torch.randperm(inputs.size(0))] for i in range(5)], dim=0)
            fir_aug = torch.cat([inputs + 0.2 * inputs[torch.randperm(inputs.size(0))] for i in range(3)], dim=0)
            inputs = torch.cat([fir_aug/(2**i) for i in range(5)], dim=0)
        if self.Ours:
            our_inputs = []
            for our_i in range(5):
                gamma = self._cal_coef(self.gamma, 'uniform', 0.0, 1.8, itr)
                eta = self._cal_coef(self.eta, 'increment_linear', 0.0, 2.2, itr)
                noise = torch.empty_like(inputs).uniform_(-self.epsilon, self.epsilon)
                our_inputs.append(torch.clamp(gamma*(inputs) + eta*noise, min=0, max=1))
            inputs = torch.cat(our_inputs, dim=0)
        return inputs

    def perturb(self, images, ori_labels, target_labels):
        b,c,h,w = images.shape
        adv_images = images.clone()
        if self.target:
            used_label = target_labels
            used_coef = -1
        else:
            used_label = ori_labels
            used_coef = 1
        
        if self.random_start:
            adv_images = adv_images + torch.empty_like(adv_images).uniform_(-self.epsilon, self.epsilon)
            adv_images = torch.clamp(adv_images, min=0, max=1).detach()
            delta = torch.tensor((adv_images - images).clone().detach(), requires_grad=True).cuda()
        else:
            delta = torch.zeros_like(adv_images,requires_grad=True).cuda()

        pert = torch.zeros_like(adv_images)
        result_matrix = np.zeros((len(self.eval_models), len(self.eval_steps)))
        iter_flag = 0
        if self.MI:
            grad_pre = 0
        if self.VT:
            vt_variance = torch.zeros_like(adv_images)

        saved_losses = []
        for itr in range(self.steps):
            delta.requires_grad_()
            self._initilize_gra_act()

            if self.RAP:
                pert = self._pgd_adv_example(self.model, adv_images+delta.detach().clone(), used_label)
            reverse_adv_inputs = adv_images + pert + delta
            
            if self.ODI:
                reverse_adv_inputs = self._ODI(reverse_adv_inputs)

            augmented_inputs = self._linear_aug(reverse_adv_inputs, itr)

            loss_label = torch.cat([used_label]*int(augmented_inputs.shape[0]/b))

            if self.DI:
                logits = self.model(self._DI(augmented_inputs))
            else:
                logits = self.model(augmented_inputs)
            
            if self.loss_fn is None:
                sum_loss = self._attention_map(logits, delta, loss_label) # num_layers, batch_size*num_augmented
                eff_all = torch.ones_like(sum_loss)
                loss = -torch.sum(eff_all * sum_loss)
            else:
                loss = self.loss_fn(logits, loss_label)

            saved_losses.append(loss.detach().clone().item())
            # loss = -sum_loss
            # loss = self.loss_fn(logits, loss_label)
            grad_c = torch.autograd.grad(loss, delta,
                                        retain_graph=False, create_graph=False)[0]
            if self.TI:
                grad_c = F.conv2d(grad_c, self.gaussian_kernel, bias=None, stride=1, padding=(2,2), groups=3)
            else:
                pass
            
            if self.VT:
                grad_c = grad_c + vt_variance

            if self.MI:
                grad_a = grad_c / torch.mean(torch.abs(grad_c), (1, 2, 3), keepdim=True) + 1 * grad_pre
                grad_pre = grad_a
            else:
                grad_a = grad_c.clone()

            delta = delta + used_coef * self.alpha * torch.sign(grad_a)
            delta = delta.clamp(-self.epsilon, self.epsilon) 
            delta = ((adv_images + delta).clamp(0,1)) - adv_images

            if self.VT:
                vt_gradients = self._vt_batch_grad(self.model, adv_images+delta.detach().clone(), used_label)
                vt_variance = vt_gradients - grad_a

            if itr+1 in self.eval_steps:
                for m_id, model in enumerate(self.eval_models):
                    with torch.no_grad():
                        this_logit = model(adv_images+delta)
                    if self.target:
                        success_nums = torch.sum(torch.argmax(this_logit, dim=1) == used_label).cpu().item()
                    else:
                        success_nums = torch.sum(torch.argmax(this_logit, dim=1) != used_label).cpu().item()
                    result_matrix[m_id, iter_flag] = success_nums
                iter_flag += 1
            torch.cuda.empty_cache()
            
        return result_matrix, adv_images+delta, saved_losses
    
